...
const APPS_DIR = dockerService.APPS_DIR;
...
router.get('/:panelId', async (req, res) => {
  try {
    ...
    // Security check
    if (!targetDir.startsWith(appDir)) {
    ...
    const files = entries
      .filter(entry => !['node_modules', 'venv', '.git', '__pycache__', '.pm2'].includes(entry.name))
      .map(entry => ({
        name: entry.name,
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (!filePath.startsWith(appDir)) {
        results.push({ path: file.path, success: false, error: 'Access denied' });
        continue;
      }
...
router.delete('/:panelId', async (req, res) => {
  try {
    ...
    // Security check
    if (!fullPath.startsWith(appDir) || fullPath === appDir) {
      return res.status(403).json({ error: 'Access denied' });
    }
